...
# ----- SAĞ SÜTUN (TELEMETRİ) - NATIVE CANVAS SCROLL (takılma yok) -----
# CTkScrollableFrame yerine tk.Canvas kullanıyoruz.
# Canvas scroll: sadece viewport offset değişir, widget re-layout olmaz.
# tk.Canvas yalnızca viewport offset'i değiştirir → hiçbir widget yeniden ölçülmez.

right_outer = ctk.CTkFrame(main, width=395, corner_radius=12,
    fg_color="#0b1320", border_width=1, border_color="#1e293b")
right_outer.grid(row=0, column=2, padx=0, pady=0, sticky="nsew")
right_outer.grid_propagate(False)
right_outer.grid_rowconfigure(0, weight=1)
right_outer.grid_columnconfigure(0, weight=1)

# Native canvas: tek gerçek anlamda takılmasız scroll widget'ı
_scroll_canvas = tk.Canvas(right_outer, bg="#0b1320", highlightthickness=0,
                            bd=0, yscrollincrement=1)
_scroll_canvas.grid(row=0, column=0, sticky="nsew")

# İnce scrollbar (6px) - görsel dokunuş
_scrollbar = tk.Scrollbar(right_outer, orient="vertical",
    command=_scroll_canvas.yview, width=6,
    bg="#0b1320", troughcolor="#050d1a",
...
# İçerik frame - tüm kartlar buraya gider
right = tk.Frame(_scroll_canvas, bg="#0b1320")
_canvas_window = _scroll_canvas.create_window((0, 0), window=right, anchor="nw")

def _on_right_configure(e):
    _scroll_canvas.configure(scrollregion=_scroll_canvas.bbox("all"))
def _on_canvas_resize(e):
    _scroll_canvas.itemconfig(_canvas_window, width=e.width)

right.bind("<Configure>", _on_right_configure)
_scroll_canvas.bind("<Configure>", _on_canvas_resize)

# ── İnertia (Atalet) Scroll Motoru ───────────────────────────────────────────
# Mouse tekerleği ani adım atmaz; hız kazanır, sürtünmeyle durur.
# Sonuç: iOS/macOS kalitesinde pürüzsüz kaydırma.
_scroll_vel  = [0.0]   # piksel/frame cinsinden anlık hız
_scroll_anim = [None]  # after() token

def _inertia_tick():
    """Her frame'de hızı sürtünmeyle azalt, canvas'ı kaydır."""
    v = _scroll_vel[0]
    if abs(v) < 0.3:
        _scroll_vel[0] = 0.0
        _scroll_anim[0] = None
        return
    _scroll_canvas.yview_scroll(int(round(v)), "pixels")
    _scroll_vel[0] *= 0.88          # sürtünme katsayısı (0.88 = doğal duruş)
    _scroll_anim[0] = app.after(8, _inertia_tick)   # ~120fps tick

def _on_mousewheel(e):
    """Scroll hızına ivme ekle (akümülatif - hızlı çevirince hızlanır)."""
    if _scroll_anim[0] is None:
        _scroll_anim[0] = app.after(8, _inertia_tick)
    # Windows: e.delta = ±120 katları | Linux: e.num 4/5
    if hasattr(e, 'delta') and e.delta != 0:
        impulse = -e.delta * 0.25          # 120 → ~30px/frame
    elif getattr(e, 'num', 0) == 4:
        impulse = -30.0
    else:
        impulse = 30.0
    _scroll_vel[0] += impulse
    # Hız üst sınırı (aşırı fırlama engeli)
    _scroll_vel[0] = max(-60.0, min(60.0, _scroll_vel[0]))

# Tüm canvas çocuklarına scroll event'i ilet
...
# Canvas ve dış frame'e global scroll bind
...
right_outer.bind("<MouseWheel>", _on_mousewheel)

# right frame'e widget eklendikçe yeni widget'lara scroll bind et
def _auto_bind_new(e):
    _bind_scroll(e.widget)
right.bind_all("<Map>", _auto_bind_new, add="+")
...
def section(parent, title, color, row):
    ...
    # Kart: tk.Frame + görsel sınır (ctk layout hatası olmaz)
    card = ctk.CTkFrame(parent, corner_radius=10, fg_color="#0d1829",
    ...
    # Scroll event'i kartlara da ilet (canvas dışındaki widget'lar için)
    for w in (hdr, lbl):
    ...
    def drag_start(e):
        card.lift()
        card.configure(border_color="#ffffff", border_width=2)
        hdr.configure(fg_color="#334155")
        card._drag_cache = [(other, other.winfo_rooty(),
                             other.winfo_rooty() + other.winfo_height())
                            for other in SECTION_FRAMES if other != card]
    ...
    def drag_stop(e):
        card.configure(border_color=card._orig_bc, border_width=1)
        hdr.configure(fg_color=card._orig_hc)
    ...
    hdr.bind("<ButtonPress-1>", drag_start); hdr.bind("<B1-Motion>", drag_motion)
    hdr.bind("<ButtonRelease-1>", drag_stop)
    lbl.bind("<ButtonPress-1>", drag_start); lbl.bind("<B1-Motion>", drag_motion)
    lbl.bind("<ButtonRelease-1>", drag_stop)
    return card
...
def data_row(parent, label, str_var, lcolor="#00ffcc", vsize=22):
    ...
    vl = ctk.CTkLabel(rf, textvariable=str_var,
        font=ctk.CTkFont(family="Consolas", size=vsize, weight="bold"),
        text_color=lcolor, anchor="e"); vl.pack(side="right")
    # Scroll event'i tüm alt widget'lara ilet
    for w in (rf, l1, vl):
        w.bind("<MouseWheel>", _on_mousewheel, add="+")
        w.bind("<Button-4>",   _on_mousewheel, add="+")
        w.bind("<Button-5>",   _on_mousewheel, add="+")
...
def div(parent):
    ...
    d.bind("<Button-4>",   _on_mousewheel, add="+")
    d.bind("<Button-5>",   _on_mousewheel, add="+")
...
# ── Başlangıçta tüm right ağacına scroll bind et ─────────────────────────────
...
# 500ms sonra tüm widget'lar render edilmiş olur
app.after(500, lambda: _bind_all_scroll(right))
