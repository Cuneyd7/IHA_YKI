...
# ══════════════════════════════════════════════════════════════
#  NAV BAR + SEKMELİ SİSTEM
# ══════════════════════════════════════════════════════════════
_aktif_tab   = [None]
_tab_frames  = {}    # ad → tk.Frame (daima place() ile render)
_tab_btns    = {}    # ad → CTkButton
_cam_labels  = []    # [lbl_kamera_small, lbl_kamera_fs, ...popup labels]

def sekme_ac(ad):
    """Anlık geçiş: lift() — Z sırası değişir, hiçbir widget redraw almaz."""
    _aktif_tab[0] = ad
    f = _tab_frames.get(ad)
    if f: f.lift()
...
_popouts = {}   # ad → Toplevel (pop-out pencereler)
_cam_popup_labels = {}   # wid → tk.Label

def pop_out(ad):
    """Sekmeyi ayrı pencereye al (kamera için gerçek içerik, diğerleri için mesaj)."""
    if ad in _popouts and _popouts[ad].winfo_exists():
        _popouts[ad].lift(); return
    titles = {"yki":"KARAN YKİ", "kamera":"FPV Kamera", "yarisma":"TEKNOFEST Yarışma"}
    win = tk.Toplevel(app)
    win.title(f"⤢  {titles.get(ad,'')}")
    win.configure(bg="#020810")
    _popouts[ad] = win

    if ad == "kamera":
        win.geometry("1280x720")
        tk.Frame(win, bg="#04080f", height=36).pack(fill="x")
        tk.Label(win.children[list(win.children)[-1]],
                 text="[ İHA FPV KAMERA ]", font=("Consolas",13,"bold"),
                 fg="#38BDF8", bg="#04080f").pack(side="left", padx=14, pady=6)
        cam_lbl = tk.Label(win, bg="#000000")
        cam_lbl.pack(fill="both", expand=True)
        wid = id(win)
        _cam_popup_labels[wid] = cam_lbl
        def _close():
            _cam_popup_labels.pop(wid, None)
            win.destroy(); _popouts.pop(ad, None)
        win.protocol("WM_DELETE_WINDOW", _close)
    else:
        win.geometry("300x120")
        tk.Label(win, text=f"{titles[ad]} ana pencerede çalışıyor.",
                 font=("Consolas",12), fg="#94a3b8", bg="#020810").pack(expand=True)
        win.protocol("WM_DELETE_WINDOW", lambda: [win.destroy(), _popouts.pop(ad,None)])

# ── Nav Bar ──────────────────────────────────────────────────
nav = ctk.CTkFrame(app, height=48, fg_color="#04080f", corner_radius=0)
nav.pack(side="top", fill="x")
nav.pack_propagate(False)

ctk.CTkLabel(nav, text="❖  KARAN İHA YER KONTROL İSTASYONU  ❖",
             font=FB, text_color="#00ffcc").pack(side="left", padx=20)

_nav_right = ctk.CTkFrame(nav, fg_color="transparent")
_nav_right.pack(side="right", padx=8)
...
for _k, _lbl in _TAB_DEFS:
    _b = ctk.CTkButton(_nav_right, text=_lbl,
        font=ctk.CTkFont(family="Consolas", size=13, weight="bold"),
        fg_color="transparent", text_color="#64748b",
        hover_color="#0f2a4a", corner_radius=8, height=32, width=160,
        command=lambda x=_k: sekme_ac(x))
    _b.pack(side="left", padx=2)
    _tab_btns[_k] = _b

for _k, _lbl in _TAB_DEFS:
    _titles = {"yki":"KARAN YKİ","kamera":"FPV Kamera","yarisma":"TEKNOFEST Yarışma"}
    ctk.CTkButton(_nav_right, text="⤢", width=30, height=32,
        font=ctk.CTkFont(family="Consolas",size=12,weight="bold"),
        fg_color="#050d1a", text_color="#38BDF8", hover_color="#1e3a5f",
        corner_radius=6, command=lambda x=_k: pop_out(x)).pack(side="left", padx=(0,4))

# ── Tab Host: tek çerçeve, 3 frame buraya place() ile ────────
_tab_host = tk.Frame(app, bg="#020810")
_tab_host.pack(fill="both", expand=True)

# YKİ Sekmesi
yki_tab = tk.Frame(_tab_host, bg="#020810")
yki_tab.place(relx=0, rely=0, relwidth=1, relheight=1)
_tab_frames["yki"] = yki_tab

main = ctk.CTkFrame(yki_tab, fg_color="transparent")
...
# ── UÇUŞ KONTROL BUTONLARI ──────────────────────────────────
_ctrl = ctk.CTkFrame(frame3d, fg_color="#050d1a", corner_radius=0, height=104,
                      border_width=1, border_color="#0f2a4a")
_ctrl.pack(fill="x", side="bottom", padx=2, pady=(0,2))
_ctrl.pack_propagate(False)

def _mav_bg(fn):
    threading.Thread(target=lambda: _safe_cmd(fn), daemon=True).start()
def _safe_cmd(fn):
    try: fn()
    except Exception as e: print(f"Cmd hatası: {e}")
def _arm():
...
def _takeoff(alt=50):
    ...
    def _t():
        baglanti.mav.command_long_send(baglanti.target_system, baglanti.target_component,
            mavutil.mavlink.MAV_CMD_NAV_TAKEOFF, 0, 0,0,0,0,0,0, alt)
    _mav_bg(_t)

_r1 = ctk.CTkFrame(_ctrl, fg_color="transparent"); _r1.pack(fill="x", padx=8, pady=(7,2))
for _i in range(6): _r1.grid_columnconfigure(_i, weight=1)
_FKB = ctk.CTkFont(family="Consolas", size=11, weight="bold")
for _ci,(_t,_fg,_hv,_cmd) in enumerate([
    ...
    ctk.CTkButton(_r1, text=_t, fg_color=_fg, hover_color=_hv, text_color="#e2e8f0",
        font=_FKB, corner_radius=6, height=32, border_width=1,
        command=_cmd).grid(row=0, column=_ci, padx=2, sticky="ew")

_r2 = ctk.CTkFrame(_ctrl, fg_color="transparent"); _r2.pack(fill="x", padx=8, pady=(0,6))
_r2.grid_columnconfigure(2, weight=1)
ctk.CTkButton(_r2, text="🔓 ARM", fg_color="#14532D", hover_color="#15803d",
    text_color="#86efac", font=_FKB, corner_radius=6, height=32,
    border_width=1, border_color="#166534", command=_arm).grid(row=0,column=0,padx=(0,3),ipadx=8)
ctk.CTkButton(_r2, text="🔒 DISARM", fg_color="#7f1d1d", hover_color="#b91c1c",
    text_color="#fca5a5", font=_FKB, corner_radius=6, height=32,
    border_width=1, border_color="#991b1b", command=_disarm).grid(row=0,column=1,padx=(0,8),ipadx=8)
ctk.CTkLabel(_r2, textvariable=SV["mode"],
    font=ctk.CTkFont(family="Consolas",size=14,weight="bold"),
    text_color="#00ffcc", fg_color="#04101a", corner_radius=6,
    anchor="center").grid(row=0, column=2, sticky="ew")
...
def master_loop():
    ...
    if EKSTRA_MODULLER_OK:
        ...
        if _mk is not None:
            ...
            # Kamera sekmesi tam ekran
            try:
                fw, fh = lbl_kamera_fs.winfo_width(), lbl_kamera_fs.winfo_height()
                if fw > 20 and fh > 20:
                    _fs = _pil.resize((fw,fh), Image.NEAREST)
                    _fs_tk = ImageTk.PhotoImage(image=_fs)
                    lbl_kamera_fs.imgtk = _fs_tk; lbl_kamera_fs.configure(image=_fs_tk)
            except: pass
            # Popup kamera pencereleri
            for _wid, _pl in list(_cam_popup_labels.items()):
                try:
                    pw,ph = _pl.winfo_width(), _pl.winfo_height()
                    if pw>20 and ph>20:
                        _pp = _pil.resize((pw,ph), Image.NEAREST)
                        _pp_tk = ImageTk.PhotoImage(image=_pp)
                        _pl.imgtk = _pp_tk; _pl.configure(image=_pp_tk)
                except: _cam_popup_labels.pop(_wid,None) 
...
#  KAMERA SEKMESİ
# ══════════════════════════════════════════════════════════════
cam_tab = tk.Frame(_tab_host, bg="#000000")
cam_tab.place(relx=0, rely=0, relwidth=1, relheight=1)
_tab_frames["kamera"] = cam_tab

_cam_hdr = tk.Frame(cam_tab, bg="#04080f", height=38)
_cam_hdr.pack(fill="x"); _cam_hdr.pack_propagate(False)
tk.Label(_cam_hdr, text="[ İHA FPV KAMERA — TAM EKRAN ]",
         font=("Consolas",13,"bold"), fg="#38BDF8", bg="#04080f").pack(side="left",padx=14,pady=7)

lbl_kamera_fs = tk.Label(cam_tab, bg="#000000")
lbl_kamera_fs.pack(fill="both", expand=True)

# Koordinat overlay
_cam_ov = tk.Frame(cam_tab, bg="#04080f", bd=1, relief="flat")
_cam_ov.place(relx=0.99, rely=0.97, anchor="se")
for _sv_key, _col in [("lat","#38BDF8"),("lon","#38BDF8"),("alt","#14B8A6"),("as","#10B981")]:
    tk.Label(_cam_ov, textvariable=SV[_sv_key],
             font=("Consolas",12,"bold"), fg=_col, bg="#04080f").pack(padx=10, pady=1)

# ══════════════════════════════════════════════════════════════
#  YARIŞMA SEKMESİ
# ══════════════════════════════════════════════════════════════
comp_tab = tk.Frame(_tab_host, bg="#020810")
comp_tab.place(relx=0, rely=0, relwidth=1, relheight=1)
_tab_frames["yarisma"] = comp_tab

# Panel bu frame içine build edilecek
_YARISMA_PARENT = comp_tab
...
# ══════════════════════════════════════════════════════════════
def _startup():
    """Tüm sekmeleri bir kez render et, sonra YKİ'yi öne al."""
    _build_panel()   # Yarışma sekmesini doldur
    app.update_idletasks()
    sekme_ac("yki")

app.after(100, telemetry_ui_loop)
app.after(200, master_loop)
app.after(250, _startup)
app.mainloop()
