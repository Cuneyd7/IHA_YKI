...
- GERÇEKÇİ UÇUŞ DİNAMİĞİ: Yük faktörü kuplajı, adverse yaw, aerodinamik sürükleme.
- BODY-FRAME CHASE-CAM: İHA'nın tam arkasından sabit göz, bankta tam sığma.
- HUD ÜST KISIM: 3D model üstte, alt kısımda uçuş kontrol butonları.
"""
...
def _hud_arka_plan():
    ...
    try:
        ...
        # ══ GERÇEKÇİ UÇUŞ DİNAMİĞİ MOTORU ═════════════════════════════
        #
        # Mimari (değiştirilen kısımlar):
        #  - RK4 integrasyon: Euler'den 4x daha hassas, birikimli hata yok (KORUNDU)
        #  - Sürekli oran enjeksiyonu: MAVLink rollspeed/pitchspeed her frame blend (KORUNDU)
        #  - Koordineli dönüş kuplajı: bank açısı → yaw görsel kıvrılma (KORUNDU)
        #  - Kesin zamanlama: perf_counter spin-wait → clock.tick jitter'ı yok (KORUNDU)
        #  - Dinamik dt cap: 20ms (50fps min) → yüksek yük altında kenetlenme yok (KORUNDU)
        #
        # YENİ EKLENENler (gerçek uçuş fiziği):
        #  - Yük faktörü kuplajı: virajda n = 1/cos(bank) → otomatik pitch-up
        #  - Adverse yaw: roll hızı → ters yaw momenti (gerçek aerodinamik)
        #  - Aerodinamik sürükleme: virajda indüklenen drag → hız düşüşü etkisi
        #  - İkinci derece pitch sönüm: pitch osilatörü hem MAVLink hem aero-kuplaj
        ...
        # ── Aerodinamik kuplaj biriktirme değişkenleri ────────────────────
        _aero_pitch_bias  = 0.0   # yük faktöründen gelen pitch kayması (rad)
        _adverse_yaw_acc  = 0.0   # adverse yaw birikimi (rad)
        ...
        while True:
            try:
                ...
                # ── MAVLink hedef açıları ─────────────────────────────────
                tgt_roll  = D.get("roll",  0.0)
                tgt_pitch = D.get("pitch", 0.0)

                # ── YENİ: Yük Faktörü Kuplajı ────────────────────────────
                # Gerçek uçuşta viraj yaparken, seviye tutmak için pitch
                # artırılmalıdır. Yük faktörü n = 1/cos(bank).
                # Bunun oluşturduğu ek pitch:
                #   delta_pitch = atan((n - 1) * g / V^2) ≈ basitleştirilmiş
                # Burada sadece görsel kuplaj yapıyoruz (MAVLink zaten gerçek
                # pitch'i gönderiyor), ama MAVLink gecikmesini kompanse eder.
                abs_roll = abs(roll_pos)
                if abs_roll > 0.05:  # 3° üstü banklarda aktif
                    cos_bank = math.cos(roll_pos)
                    cos_bank = max(cos_bank, 0.35)  # 70° üstü için sınır
                    load_factor = 1.0 / cos_bank
                    # Yük faktörü → pitch kayması: virajda burun doğal olarak
                    # düşer, pilot pitch-up yapar. Bunu kuplaj olarak ekle.
                    pitch_load_bias = (load_factor - 1.0) * 0.035  # ~2° @ 45° bank
                else:
                    pitch_load_bias = 0.0

                # Yumuşak geçiş (tau=80ms)
                k_aero = 1.0 - math.exp(-dt / 0.080)
                _aero_pitch_bias += (pitch_load_bias - _aero_pitch_bias) * k_aero

                # Kuplajlı pitch hedefi: MAVLink değeri + aerodinamik bias
                tgt_pitch_coupled = tgt_pitch + _aero_pitch_bias

                # ── YENİ: Adverse Yaw ────────────────────────────────────
                # Gerçek uçakta aileron sürüklenmesi → roll yönünün TERSİNE
                # yaw momenti oluşturur. Rudder ile düzeltilir.
                # Burada roll hızından adverse yaw hesaplanır:
                #   adverse_yaw ∝ -rollspeed * k_adverse
                # İşaret: sağ roll (pozitif rollspeed) → sol yaw (negatif)
                adverse_yaw_moment = -roll_vel * 0.08  # küçük ama hissedilir
                k_adv = 1.0 - math.exp(-dt / 0.060)
                _adverse_yaw_acc += (adverse_yaw_moment - _adverse_yaw_acc) * k_adv

                # ── RK4 ile uçuş dinamiği entegrasyonu ───────────────────
                roll_pos,  roll_vel  = rk4(roll_pos,  roll_vel,
                                           tgt_roll, ROLL_OMEGA,  ROLL_ZETA,  dt)
                pitch_pos, pitch_vel = rk4(pitch_pos, pitch_vel,
                                           tgt_pitch_coupled, PITCH_OMEGA, PITCH_ZETA, dt)

                # ── Koordineli dönüş + Adverse Yaw → Görsel Yaw ─────────
                # Kuyruğun arkasından bakınca:
                #   - Koordineli dönüş: sağ banka → burun sağa (bank * 0.12)
                #   - Adverse yaw: sağ roll → burun SOLA (ters) → pilotun
                #     rudder ile düzelttiği küçük kayma
                # İkisi birlikte gerçekçi yaw hareketini oluşturur
                coordinated_yaw = roll_pos * 0.12 + _adverse_yaw_acc
                yaw_vis, yaw_vel = rk4(yaw_vis, yaw_vel,
                                       coordinated_yaw, YAW_OMEGA, YAW_ZETA, dt)

                # ── Body-Frame Chase-Cam (GENİŞ FOV, YAKIŞTIRAN MESAFE) ──
                #
                ...
                #
                # DEĞİŞİKLİK: FOV 50→42°, mesafe 2.6→2.2
                #   - Daha dar FOV = model ekranı daha iyi doldurur
                #   - Daha yakın mesafe = kanatlar bankta bile görünür
                #   - Offset_y 0.28→0.22 = model biraz yukarı kayar (butonlara yer)
                #
                # İşaret kuralları (kuyruğun arkasından bakış):
                #   Roll+  → sağ kanat AŞAĞI  → glRotate(-roll, Z)
                #   Pitch+ → burun YUKARI      → glRotate(+pitch, X)
                #   Yaw    → koordineli dönüş  → glRotate(yaw*0.25, Y) hafif
                glViewport(0, 0, HUD_W, HUD_H)
                glClearColor(0.02, 0.04, 0.10, 1.0)
                glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
                glMatrixMode(GL_PROJECTION); glLoadIdentity()

                # ── Dinamik FOV: bankta hafif genişlet, kanatlar kesmesin ─
                base_fov = 42.0
                bank_abs_deg = abs(math.degrees(roll_pos))
                # 30° üstü banklarda FOV hafif açılır (max +6° @ 60° bank)
                fov_expand = max(0.0, (bank_abs_deg - 30.0)) * 0.20
                fov_expand = min(fov_expand, 6.0)
                current_fov = base_fov + fov_expand

                gluPerspective(current_fov, HUD_W / HUD_H, 0.3, 200.0)
                glMatrixMode(GL_MODELVIEW); glLoadIdentity()

                # Kamera body-frame offset:
                #   Z=-2.2 → modelin 2.2 birim arkasında (yakın = büyük model)
                #   Y=-0.22 → modelin 0.22 üstünde (model üst yarıda, alta yer)
                glTranslatef(0.0, -0.22, -2.2)

                # Attitude dönüşleri
                # Yaw: koordineli dönüş + adverse yaw → çok küçük görsel kayma
                glRotatef(math.degrees(yaw_vis * 0.25), 0, 1, 0)
                # Pitch: MAVLink pitch + yük faktörü kuplajı
                glRotatef(math.degrees( pitch_pos),     1, 0, 0)
                # Roll: gerçek roll (sağ kanat aşağı = pozitif)
...
# ═════════════════════════════════════════════════════════════════
#  ORTA SÜTUN: 3D HUD (ÜST) + UÇUŞ KONTROL BUTONLARI (ALT)
# ═════════════════════════════════════════════════════════════════
# Orta sütun artık iki parçalı:
#   Üst: 3D model HUD (genişleyebilir, weight=1)
#   Alt: Kontrol butonları paneli (sabit yükseklik)

mid_panel = ctk.CTkFrame(main, fg_color="transparent")
mid_panel.grid(row=0, column=1, padx=(0,10), pady=0, sticky="nsew")
mid_panel.grid_rowconfigure(0, weight=1)    # HUD genişler
mid_panel.grid_rowconfigure(1, weight=0)    # Butonlar sabit
mid_panel.grid_columnconfigure(0, weight=1)

# ── 3D HUD Frame (üst kısım - genişleyebilir) ────────────────────
frame3d = ctk.CTkFrame(mid_panel, corner_radius=12, fg_color="#040810", border_width=2, border_color="#00ffcc")
frame3d.grid(row=0, column=0, padx=0, pady=(0,5), sticky="nsew")

if OPENGL_OK:
    lbl_hud = tk.Label(frame3d, bg="#040810"); lbl_hud.pack(fill="both", expand=True, padx=2, pady=(8,2))

# Placeholder değişkenler (master_loop referansları için)
...
# ── UÇUŞ KONTROL BUTONLARI PANELİ (alt kısım - sabit) ───────────
btn_panel = ctk.CTkFrame(mid_panel, height=120, corner_radius=12, fg_color="#040810", border_width=2, border_color="#00ffcc")
btn_panel.grid(row=1, column=0, padx=0, pady=(5,0), sticky="ew")
btn_panel.grid_propagate(False)

# Başlık
btn_hdr = ctk.CTkFrame(btn_panel, fg_color="transparent")
btn_hdr.pack(fill="x", padx=6, pady=(4,2))
ctk.CTkLabel(btn_hdr, text="[ UÇUŞ KONTROL ]", font=FK, text_color="#00ffcc").pack(side="left", padx=6)

# Mod değişikliği durumu etiketi
lbl_cmd_status = ctk.CTkLabel(btn_hdr, text="", font=ctk.CTkFont(family="Consolas", size=10),
                               text_color="#94a3b8")
lbl_cmd_status.pack(side="right", padx=6)

def _cmd_status(msg, color="#10B981"):
    """Buton panelindeki durum mesajını günceller, 3sn sonra temizler."""
    lbl_cmd_status.configure(text=msg, text_color=color)
    lbl_cmd_status.after(3000, lambda: lbl_cmd_status.configure(text=""))

# ── Mod Butonları (Satır 1) ──────────────────────────────────────
btn_row1 = ctk.CTkFrame(btn_panel, fg_color="transparent")
btn_row1.pack(fill="x", padx=10, pady=(2,2))

UCUS_MODLARI = [
    ("MANUAL",  "#ef4444", "#7f1d1d"),
    ("FBWA",    "#f59e0b", "#78350f"),
    ("FBWB",    "#f59e0b", "#78350f"),
    ("AUTO",    "#10b981", "#064e3b"),
    ("RTL",     "#3b82f6", "#1e3a8a"),
    ("LOITER",  "#8b5cf6", "#4c1d95"),
]

def _mod_degistir(mod_adi):
    """MAVLink ile uçuş modunu değiştirir."""
    if baglanti:
        try:
            mod_id = baglanti.mode_mapping().get(mod_adi, -1)
            if mod_id >= 0:
                baglanti.set_mode(mod_id)
                _cmd_status(f"✓ {mod_adi} modu gönderildi", "#10B981")
            else:
                _cmd_status(f"✗ {mod_adi} bulunamadı", "#ef4444")
        except Exception as e:
            _cmd_status(f"✗ Hata: {e}", "#ef4444")
    else:
        _cmd_status("✗ MAVLink bağlantısı yok", "#ef4444")

for mod_adi, renk, bg_renk in UCUS_MODLARI:
    b = ctk.CTkButton(btn_row1, text=mod_adi,
        font=ctk.CTkFont(family="Consolas", size=11, weight="bold"),
        width=70, height=28, corner_radius=6,
        fg_color=bg_renk, hover_color=renk, text_color="#ffffff",
        border_width=1, border_color=renk,
        command=lambda m=mod_adi: _mod_degistir(m))
    b.pack(side="left", padx=3, expand=True)

# ── Komut Butonları (Satır 2) ────────────────────────────────────
btn_row2 = ctk.CTkFrame(btn_panel, fg_color="transparent")
btn_row2.pack(fill="x", padx=10, pady=(2,6))

def _arm_komutu():
    if baglanti:
        try:
            baglanti.arducopter_arm()
            _cmd_status("✓ ARM komutu gönderildi", "#10B981")
        except:
            try:
                ...
                _cmd_status(f"✗ ARM hatası: {e}", "#ef4444")
    else:
        _cmd_status("✗ MAVLink bağlantısı yok", "#ef4444")
...
def _disarm_komutu():
    if baglanti:
        try:
            baglanti.arducopter_disarm()
            _cmd_status("✓ DISARM komutu gönderildi", "#f59e0b")
        except:
            try:
                baglanti.mav.command_long_send(
                    baglanti.target_system, baglanti.target_component,
                ...
                _cmd_status(f"✗ DISARM hatası: {e}", "#ef4444")
    else:
        _cmd_status("✗ MAVLink bağlantısı yok", "#ef4444")
...
def _takeoff_komutu():
    if baglanti:
        try:
            ...
            _cmd_status("✓ TAKEOFF komutu gönderildi (20m)", "#10B981")
        except Exception as e:
            _cmd_status(f"✗ TAKEOFF hatası: {e}", "#ef4444")
    else:
        _cmd_status("✗ MAVLink bağlantısı yok", "#ef4444")
...
def _land_komutu():
    if baglanti:
        try:
            ...
            _cmd_status("✓ LAND komutu gönderildi", "#3b82f6")
        except Exception as e:
            _cmd_status(f"✗ LAND hatası: {e}", "#ef4444")
    else:
        _cmd_status("✗ MAVLink bağlantısı yok", "#ef4444")

KOMUT_BUTONLARI = [
    ("⚡ ARM",     _arm_komutu,     "#10B981", "#064e3b"),
    ("◼ DISARM",  _disarm_komutu,  "#ef4444", "#7f1d1d"),
    ("▲ TAKEOFF", _takeoff_komutu, "#3b82f6", "#1e3a8a"),
    ("▼ LAND",    _land_komutu,    "#f59e0b", "#78350f"),
]

for txt, cmd, renk, bg_renk in KOMUT_BUTONLARI:
    b = ctk.CTkButton(btn_row2, text=txt,
