...
def sekme_ac(ad):
    """Aktif sekmeyi değiştirir — place+lift ile ANİ geçiş, sıfır layout hesabı."""
    aktif_sekme[0] = ad
    # Buton renklerini anlık güncelle
    ...
    # Pop-out penceresi zaten açıksa oraya odaklan
    if ad in _popout_windows:
        try:
            if _popout_windows[ad].winfo_exists():
                _popout_windows[ad].lift()
                _popout_windows[ad].focus_force()
                return
        except Exception:
            pass
        _popout_windows.pop(ad, None)

    # Tüm frame'leri yerleştir ve sadece aktif olanı öne getir
    # place() kullandığımız için layout hesabı yapılmaz → kastırma yok
    def _switch():
        for k, f in sekme_frames.items():
            if k in _popout_windows:
                try:
                    if _popout_windows[k].winfo_exists():
                        continue   # popout'ta açık, ana pencerede gösterme
                except Exception:
                    _popout_windows.pop(k, None)
            try:
                f.place(relx=0, rely=0, relwidth=1, relheight=1)
            except Exception:
                pass
        aktif_f = sekme_frames.get(ad)
        if aktif_f:
            aktif_f.lift()
    app.after(0, _switch)
...
def pop_out(ad, title):
    """Sekmeyi ayrı bir Toplevel penceresinde gösterir.
    Aynı anda birden fazla popout desteklenir — siyah ekran oluşmaz.
    Teknik: frame place(in_=win) ile ayrı pencerede render edilir,
    ana pencere ise başka sekmeye geçer."""
    ...
    # Frame'i bu pencerede göster (place ile)
    f.place_forget()
    f.place(in_=win, relx=0, rely=0, relwidth=1, relheight=1)
    f.lift()

    # Ana pencereyi başka sekmeye yönlendir (siyah ekran önleme)
    _diger_sekme = [k for k in sekme_frames if k != ad and k not in _popout_windows]
    if _diger_sekme:
        app.after(50, lambda: sekme_ac(_diger_sekme[0]))
    else:
        # Tüm sekmeler popout'ta, placeholder göster
        pass

    def on_close():
        try:
            # Frame'i ana pencereye geri al
            f.place_forget()
            f.place(relx=0, rely=0, relwidth=1, relheight=1)
        except Exception:
            pass
        _popout_windows.pop(ad, None)
        win.destroy()
        # Geri dönerken bu sekmeye geç
        app.after(0, lambda: sekme_ac(ad))
...
# ── UÇUŞ KONTROL BUTONLARI (HUD alt kısım) — Mission Planner Tarzı ───────────
ctrl_bar = ctk.CTkFrame(frame3d, fg_color="#030710",
                         corner_radius=0, height=190,
...
def _land():
    if baglanti: _mav_bg(lambda: baglanti.set_mode("LAND"))

def _do_change_speed(delta):
    """Hava hızını delta m/s kadar değiştir (MAV_CMD_DO_CHANGE_SPEED)."""
    if not baglanti: return
    def _s():
        spd = max(5.0, D.get("airspeed", 15.0) + delta)
        baglanti.mav.command_long_send(
            baglanti.target_system, baglanti.target_component,
...
def _do_change_alt(delta):
    """Hedef irtifayı delta metre kadar değiştir."""
    if not baglanti: return
    def _a():
        new_alt = max(10.0, _agl_val[0] + delta)
        baglanti.mav.command_long_send(
            baglanti.target_system, baglanti.target_component,
...
def _mission_start():
    """AUTO moduna geç ve görev başlat."""
    if not baglanti: return
    _mav_bg(lambda: baglanti.set_mode("AUTO"))

def _mission_pause():
    """Görevi durdur — LOITER'a geç."""
    if baglanti: _mav_bg(lambda: baglanti.set_mode("LOITER"))

_FKB = ctk.CTkFont(family="Consolas", size=11, weight="bold")
_FKBS = ctk.CTkFont(family="Consolas", size=10, weight="bold")

# ── Başlık şeridi ──────────────────────────────────────────────
_hdr = ctk.CTkFrame(ctrl_bar, fg_color="#04101a", height=20, corner_radius=0)
_hdr.pack(fill="x", padx=0, pady=(0,2))
ctk.CTkLabel(_hdr, text="◈  UÇUŞ KONTROL PANELİ  ◈",
             font=ctk.CTkFont(family="Consolas", size=10, weight="bold"),
             text_color="#1e5f8a").pack(pady=1)

# ── Satır 1: Uçuş Modları (6 eşit buton) ──────────────────────
_r1 = ctk.CTkFrame(ctrl_bar, fg_color="transparent")
_r1.pack(fill="x", padx=6, pady=(2,2))
for i in range(6): _r1.grid_columnconfigure(i, weight=1)

_MODS = [
    ("✈ AUTO",    "#0f2952","#2563eb", lambda:_set_mode("AUTO")),
    ("🎯 GUIDED", "#0f2952","#2563eb", lambda:_set_mode("GUIDED")),
    ("⟳ LOITER",  "#05311e","#059669", lambda:_set_mode("LOITER")),
    ("🏠 RTL",    "#4a1208","#c2410c", lambda:_set_mode("RTL")),
    ("🕹 MANUAL", "#131320","#334155", lambda:_set_mode("MANUAL")),
    ("⬆ TAKEOFF", "#1a0840","#7c3aed", lambda:_takeoff(50)),
]
for c,(txt,fg,hv,cmd) in enumerate(_MODS):
    ctk.CTkButton(_r1, text=txt, fg_color=fg, hover_color=hv,
                  text_color="#e2e8f0", font=_FKB,
                  corner_radius=5, height=32, border_width=1,
...
_r2.pack(fill="x", padx=6, pady=(0,2))
for i in range(7): _r2.grid_columnconfigure(i, weight=1)

ctk.CTkButton(_r2, text="🔓 ARM", fg_color="#0d3b20", hover_color="#15803d",
              text_color="#86efac", font=_FKB, corner_radius=5, height=32,
              border_width=1, border_color="#166534",
              command=_arm).grid(row=0, column=0, padx=2, sticky="ew")

ctk.CTkButton(_r2, text="🔒 DISARM", fg_color="#4a0e0e", hover_color="#b91c1c",
              text_color="#fca5a5", font=_FKB, corner_radius=5, height=32,
              border_width=1, border_color="#7f1d1d",
              command=_disarm).grid(row=0, column=1, padx=2, sticky="ew")

ctk.CTkLabel(_r2, textvariable=SV["mode"],
    font=ctk.CTkFont(family="Consolas", size=13, weight="bold"),
    text_color="#00ffcc", fg_color="#04101a",
    corner_radius=5, anchor="center"
    ).grid(row=0, column=2, columnspan=3, padx=2, sticky="ew")

# Hız & İrtifa mini göstergesi (sağ köşe)
_spd_mini = ctk.CTkFrame(_r2, fg_color="#04101a", corner_radius=5)
_spd_mini.grid(row=0, column=5, columnspan=2, padx=2, sticky="ew")
ctk.CTkLabel(_spd_mini, textvariable=SV["as"],
    font=ctk.CTkFont(family="Consolas", size=11, weight="bold"),
    text_color="#38BDF8", anchor="center").pack(side="left", padx=6, pady=2)
ctk.CTkLabel(_spd_mini, textvariable=SV["agl"],
    font=ctk.CTkFont(family="Consolas", size=11, weight="bold"),
    text_color="#14B8A6", anchor="center").pack(side="right", padx=6, pady=2)

# ── Satır 3: Navigasyon & Hız/İrtifa Kontrolleri ──────────────
_r3 = ctk.CTkFrame(ctrl_bar, fg_color="transparent")
_r3.pack(fill="x", padx=6, pady=(0,4))
for i in range(7): _r3.grid_columnconfigure(i, weight=1)

_NAV = [
    ("▼ HIZ",   "#0f2038","#1e4060", lambda:_do_change_speed(-2),  "−2 m/s"),
    ("▼ İRTİFA","#042a1e","#0a5c3c", lambda:_do_change_alt(-10),   "−10 m"),
    ("⏸ PAUSE", "#3a1a00","#7c3000", _mission_pause,               "LOITER"),
    ("⏹ LAND",  "#3a0a0a","#8b1a1a", _land,                        "İniş"),
    ("▶ GÖREV", "#0a2a0a","#1a6b1a", _mission_start,               "AUTO"),
    ("▲ İRTİFA","#042a1e","#0a5c3c", lambda:_do_change_alt(+10),   "+10 m"),
    ("▲ HIZ",   "#0f2038","#1e4060", lambda:_do_change_speed(+2),  "+2 m/s"),
]
for c,(txt,fg,hv,cmd,tip) in enumerate(_NAV):
    btn = ctk.CTkButton(_r3, text=txt, fg_color=fg, hover_color=hv,
                  text_color="#e2e8f0", font=_FKBS,
                  corner_radius=5, height=30, border_width=1,
                  command=cmd)
...
def _init_sekme_layout():
    """Tüm sekme frame'lerini tab_container'a place ile yerleştir.
    Bu sayede sekme_ac sadece lift() çağırır — layout hesabı sıfır, kastırma yok."""
    try:
...
app.after(80, _init_sekme_layout)
